...
#define SPDRP_MAXIMUM_PROPERTY            (0x00000025)  // Upper bound on ordinals
	...
	typedef HDEVINFO (__stdcall *SetupDiGetClassDevsA_)(CONST GUID *ClassGuid, PCSTR Enumerator, HWND hwndParent, DWORD Flags);
	typedef BOOL (__stdcall *SetupDiEnumDeviceInterfaces_)(HDEVINFO DeviceInfoSet, PSP_DEVINFO_DATA DeviceInfoData, CONST GUID *InterfaceClassGuid, DWORD MemberIndex, PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData);
	typedef BOOL (__stdcall *SetupDiGetDeviceInterfaceDetailA_)(HDEVINFO DeviceInfoSet, PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData, PSP_DEVICE_INTERFACE_DETAIL_DATA_A DeviceInterfaceDetailData, DWORD DeviceInterfaceDetailDataSize, PDWORD RequiredSize, PSP_DEVINFO_DATA DeviceInfoData);
	typedef BOOL (__stdcall *SetupDiEnumDeviceInfo_)(HDEVINFO DeviceInfoSet, DWORD MemberIndex, PSP_DEVINFO_DATA DeviceInfoData);
	typedef BOOL (__stdcall *SetupDiGetDeviceRegistryPropertyA_)(HDEVINFO DeviceInfoSet, PSP_DEVINFO_DATA DeviceInfoData, DWORD Property, PDWORD PropertyRegDataType, PBYTE PropertyBuffer, DWORD PropertyBufferSize, PDWORD RequiredSize);
	typedef BOOL (__stdcall *SetupDiDestroyDeviceInfoList_)(HDEVINFO DeviceInfoSet);
	...
	static SetupDiGetClassDevsA_ SetupDiGetClassDevsA;
	static SetupDiEnumDeviceInterfaces_ SetupDiEnumDeviceInterfaces;
	static SetupDiGetDeviceInterfaceDetailA_ SetupDiGetDeviceInterfaceDetailA;
	static SetupDiEnumDeviceInfo_ SetupDiEnumDeviceInfo;
	static SetupDiGetDeviceRegistryPropertyA_ SetupDiGetDeviceRegistryPropertyA;
	static SetupDiDestroyDeviceInfoList_ SetupDiDestroyDeviceInfoList;

	static HMODULE hid_lib_handle = NULL;
	static HMODULE setupapi_lib_handle = NULL;
...
#endif
#define RESOLVE(lib_handle, x) x = (x##_)GetProcAddress(lib_handle, #x); if (!x) return -1;
	hid_lib_handle = LoadLibraryA("hid.dll");
	if (hid_lib_handle) {
	...
	setupapi_lib_handle = LoadLibraryA("setupapi.dll");
	if (setupapi_lib_handle) {
		RESOLVE(setupapi_lib_handle, SetupDiGetClassDevsA);
		RESOLVE(setupapi_lib_handle, SetupDiEnumDeviceInterfaces);
		RESOLVE(setupapi_lib_handle, SetupDiGetDeviceInterfaceDetailA);
		RESOLVE(setupapi_lib_handle, SetupDiEnumDeviceInfo);
		RESOLVE(setupapi_lib_handle, SetupDiGetDeviceRegistryPropertyA);
		RESOLVE(setupapi_lib_handle, SetupDiDestroyDeviceInfoList);
	}
	else
		return -1;
#undef RESOLVE
...
int HID_API_EXPORT hid_exit(void)
{
	...
	hid_lib_handle = NULL;
	if (setupapi_lib_handle)
		FreeLibrary(setupapi_lib_handle);
	setupapi_lib_handle = NULL;
	initialized = FALSE;
#endif
	return 0;
}
